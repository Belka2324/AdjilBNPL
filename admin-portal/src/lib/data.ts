...
type AuditAction = 'suspend' | 'reactivate' | 'delete' | 'blacklist' | 'unblacklist' | 'freeze' | 'unfreeze' | 'activate' | 'create' | 'update'
...
export const auditLog = async (
  ...
  const entry: AuditLogEntry = {
    ...
    action,
    details,
    performed_by: session?.email || 'unknown'
  }

  if (hasSupabase && supabase) {
    // Try to insert into admin_audit_logs table matching schema expectations
    ...
    if (error) {
      ...
      console.warn('[AUDIT]', new Date().toISOString(), entry)
    }
  } else {
...
const getSession = (): { email?: string } | null => {
  try {
    const sessionStr = localStorage.getItem('adjil_session')
...
export const fetchStaff = async (): Promise<StaffRecord[]> => {
  ...
  if (hasSupabase && supabase) {
    ...
    console.log('Fetched staff from Supabase:', data?.length, 'records')
    if (data && data.length > 0) {
      setCache(CACHE_KEYS.staff, data)
      return data
    }
    console.log('No staff data from Supabase, using demo')
  }
...
export const createStaff = async (member: Omit<StaffRecord, 'id' | 'created_at' | 'updated_at' | 'reports_count' | 'messages_count'>): Promise<void> => {
  if (hasSupabase && supabase) {
    // Map first_name + last_name to full_name for database
    const dbMember: Record<string, unknown> = {
      ...
      is_ceo: member.role === 'ceo' ? true : (member.is_ceo ?? false)
    }
    
    // Only add optional fields if they have values
    if (member.phone) dbMember.phone = member.phone
    if (member.institution) dbMember.institution = member.institution
    
    console.log('Creating staff with data:', dbMember)
...
export const finalizePendingTransactions = async (transactionIds: string[]): Promise<void> => {
  if (hasSupabase && supabase && transactionIds.length > 0) {
    const { error } = await supabase
      .from('transactions')
...
// Demo data for development - only customers and merchants (staff is in staff table)
