...
export default function Users({ isAdmin = false }: Props) {
  ...
  const [showAllRoles, setShowAllRoles] = useState(isAdmin) // Show all roles for admin/CEO

  useEffect(() => {
    fetchUsers().then((data) => {
      // Filter out merchants for regular users, show all for admin/CEO
      if (isAdmin) {
        setUsers(data) // Show all users including merchants, admins, partners, support
      } else {
          ...
          <table className="w-full text-sm">
            <thead className="text-slate-400">
              <tr>
                ...
                <th className="text-right py-2">{showAllRoles ? 'الدور' : 'الباقة'}</th>
                  ...
                  <td className="py-3">
                    {showAllRoles ? (
                      <span className={`px-2 py-1 rounded text-xs font-medium ${
                        u.role === 'admin' || u.role === 'administrator' || u.role === 'ceo' ? 'bg-purple-100 text-purple-700' :
                        u.role === 'partner' ? 'bg-emerald-100 text-emerald-700' :
                        u.role === 'support' ? 'bg-blue-100 text-blue-700' :
                        u.role === 'merchant' ? 'bg-orange-100 text-orange-700' :
                        'bg-slate-100 text-slate-700'
                      }`}>
                        {u.role === 'admin' || u.role === 'administrator' || u.role === 'ceo' ? 'مدير / Admin' :
                         u.role === 'partner' ? 'شريك / Partner' :
                         u.role === 'support' ? 'دعم / Support' :
                         u.role === 'merchant' ? 'تاجر / Merchant' :
                         u.role === 'customer' ? 'زبون / Customer' :
                         u.role || '—'}
                      </span>
                    ) : (
                      u.subscription_plan || '—'
