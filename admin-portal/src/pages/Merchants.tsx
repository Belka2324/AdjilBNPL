...
import { fetchUsers, fetchTransactions, updateUserStatus, addToBlacklist } from '../lib/data'
...
export default function Merchants({ isAdmin = false }: Props) {
  ...
  // Filter transactions for selected merchant
  const merchantTxs = useMemo(() => {
    if (!selected) return []
    return transactions.filter((t) => t.merchant_id === selected.id || t.merchant_name === selected.name)
  }, [selected, transactions])

  // Calculate merchant stats
  const merchantStats = useMemo(() => {
    if (!selected) return { totalSales: 0, pending: 0, completed: 0, count: 0 }
    const mTxs = merchantTxs
    return {
      totalSales: mTxs.reduce((sum, t) => sum + (Number(t.amount) || 0), 0),
      pending: mTxs.filter((t) => t.status === 'pending').reduce((sum, t) => sum + (Number(t.amount) || 0), 0),
      completed: mTxs.filter((t) => t.status === 'completed').reduce((sum, t) => sum + (Number(t.amount) || 0), 0),
      count: mTxs.length
    }
  }, [selected, merchantTxs])
      ...
      <div className="nexus-card p-6 space-y-4">
        ...
        {selected ? (
          <div className="space-y-3 text-sm">
            ...
            <div className="pt-3 border-t border-slate-100">
              <div className="font-semibold mb-2">المبيعات / المعاملات</div>
              <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                <div className="bg-slate-50 p-2 rounded">
                  <div className="text-slate-500">إجمالي المبيعات</div>
                  <div className="font-bold text-green-600">{merchantStats.totalSales.toLocaleString('fr-DZ')} دج</div>
                </div>
                <div className="bg-slate-50 p-2 rounded">
                  <div className="text-slate-500">عدد المعاملات</div>
                  <div className="font-bold">{merchantStats.count}</div>
                </div>
                <div className="bg-slate-50 p-2 rounded">
                  <div className="text-slate-500">مكتملة</div>
                  <div className="font-bold text-emerald-600">{merchantStats.completed.toLocaleString('fr-DZ')} دج</div>
                </div>
                <div className="bg-slate-50 p-2 rounded">
                  <div className="text-slate-500">قيد الانتظار</div>
                  <div className="font-bold text-orange-600">{merchantStats.pending.toLocaleString('fr-DZ')} دج</div>
                </div>
              </div>
              ...
              {merchantTxs.length > 0 ? (
                <div className="max-h-48 overflow-auto">
                  <table className="w-full text-xs">
                    <thead className="text-slate-400 sticky top-0 bg-white">
                      <tr>
                        ...
                        <th className="text-right py-1">الزبون</th>
                        <th className="text-right py-1">المبلغ</th>
                        <th className="text-right py-1">الحالة</th>
                      </tr>
                    </thead>
                    <tbody>
                      {merchantTxs.slice(0, 10).map((t) => (
                        <tr key={t.id} className="border-t border-slate-100">
                          <td className="py-1">{t.created_at ? new Date(t.created_at).toLocaleDateString('ar-DZ') : '—'}</td>
                          <td className="py-1">{t.customer_name || '—'}</td>
                          <td className="py-1">{Number(t.amount || 0).toLocaleString('fr-DZ')} دج</td>
                          <td className="py-1">
                            <span className={`px-1 py-0.5 rounded text-xs ${
                              ...
                              {t.status === 'completed' ? 'مكتمل' : t.status === 'pending' ? 'قيد الانتظار' : 'فشل'}
                            </span>
                          </td>
                        </tr>
                      ))}
                  ...
                  {merchantTxs.length > 10 && (
                    <div className="text-xs text-slate-400 mt-1">+ {merchantTxs.length - 10} معاملة أخرى</div>
                  )}
                </div>
              ) : (
                <div className="text-xs text-slate-400">لا توجد معاملات</div>
              )}
            </div>
          </div>
        ) : (
