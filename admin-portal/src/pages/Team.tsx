...
import { fetchStaff, createStaff, INSTITUTIONS } from '../lib/data'
...
export default function Team() {
  ...
  // Get current session to check CEO status
  const session = getSession()
  const isCEO = session?.isCEO === true || session?.role === 'ceo'
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newMember = {
      ...
      institution: formData.get('institution') as string || 'Adjil HQ',
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-nexus-gold/20 flex items-center justify-center text-nexus-gold text-xs font-black">
                          {(member.first_name || member.full_name || '?')[0]}{(member.last_name || '')[0]}
                        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-6" onClick={() => setShowAddModal(false)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl p-8 space-y-6" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500">الهاتف / Phone</label>
                <input name="phone" className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-nexus-gold/50" placeholder="+213 XXX XXX XXX" />
              </div>
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500">كلمة المرور / Password</label>
                <input name="password" type="password" required className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-nexus-gold/50" placeholder="********" />
              </div>
