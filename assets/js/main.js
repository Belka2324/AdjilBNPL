...
const app = {
                ...
                txList.innerHTML = app.user.txs.map(tx => `
                    <div onclick="app.showInvoiceModal('${tx.id}')" class="flex items-center justify-between p-4 bg-slate-900/30 rounded-2xl border border-white/5 hover:bg-white/5 transition-colors cursor-pointer">
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-[8px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-400 inline-block mt-1">${t.status_success}</div>
                        </div>
                    </div>
                `).reverse().join('');
            } else {
            ...
            if (completedTxs.length > 0) {
                html += completedTxs.map(tx => `
                    <div onclick="app.showInvoiceModal('${tx.id}')" class="flex items-center justify-between p-4 bg-slate-900/50 rounded-2xl border border-slate-800 cursor-pointer hover:bg-slate-800 transition-colors">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 ${tx.status === 'pending' && tx.method === 'API_SETTLEMENT' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-green-500/10 text-green-500'} rounded-full flex items-center justify-center">
                        ...
                        <div class="text-right">
                            <div class="text-white font-black text-sm">+${tx.amount} دج</div>
                            <div class="text-[8px] px-1.5 py-0.5 rounded ${tx.status === 'pending' && tx.method === 'API_SETTLEMENT' ? 'bg-yellow-500/20 text-yellow-500' : 'bg-blue-500/20 text-blue-400'} inline-block mt-1">${tx.status === 'pending' && tx.method === 'API_SETTLEMENT' ? (t.collecting || 'Collecting') : (t.status_success || 'Success')}</div>
                        </div>
                    </div>
                `).join('');
            } else {
    ...
    dueAmountOk: () => {
        ...
        if (merchantId && merchantId.length > 5) {
            ...
            // Bypass PIN UI
            const pinContainer = document.getElementById('pay-pin')?.parentElement;
            if (pinContainer) pinContainer.style.display = 'none';
            
            const titleEl = document.querySelector('#pin-verification-content h3');
            const descEl = document.querySelector('#pin-verification-content p');
            if (titleEl) titleEl.textContent = app.lang === 'ar' ? 'تأكيد تفاصيل الدفع' : 'Confirm Payment Details';
            if (descEl) descEl.textContent = app.lang === 'ar' ? `سيتم خصم ${amount} دج لصالح متجر ${merchant.name}` : `${amount} DZD will be deducted for ${merchant.name}`;
    ...
    confirmPinPayment: () => {
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                document.getElementById('failure-payment-content').classList.remove('hidden');
                
                const reasonEl = document.getElementById('failure-reason');
                if (reasonEl) reasonEl.textContent = err.message || (app.lang === 'ar' ? 'حدث خطأ غير متوقع' : 'Unexpected error occurred');
            ...
            // Restore PIN UI for future manual txs
            const pinContainer = document.getElementById('pay-pin')?.parentElement;
